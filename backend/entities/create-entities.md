...
migrations [in the Update Database Schema section](#book-entities-database-schema-update). To run migrations
and emulate complete migration process, use the `oro:platform:update` command.

<a id="book-entities-doctrine"></a>
...
#### NOTE
The names of the schema version directories are compared to each other using PHP’s <a href="https://www.php.net/manual/en/function.version-compare.php" target="_blank">version_compare</a> function. Therefore, it is a good practice to name them  following the `v1_0`, `v2_0` pattern.

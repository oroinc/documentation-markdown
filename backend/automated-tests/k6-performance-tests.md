...
k6 is an open-source load-testing tool for APIs, microservices, and websites. Developers and testers use it to simulate traffic to a system and analyze its performance under various conditions. Its key features include:
...
This guide explains how to run performance and load tests on the OroCommerce application with k6.
...
After the tests run, a summary of the results is available in `performance/summary.html`. Generated by `k6-reporter`, it provides detailed metrics on the performance and load handling of the OroCommerce application.
...
Average response time is the average time to complete a request. Use the `http_req_duration` metric in the report to measure this.

#### Peak Response Time

Peak response time is the maximum time to complete a request. Use the `http_req_duration` metric in the report to find the maximum value.

#### Error Rate

Error rate is the percentage of requests that result in an error. Use the `check_failure_rate` metric in the report to measure this.
...
Concurrent users is the number of users simultaneously connected to your application. In the provided HTML file, this information is given in the title of each tab (e.g., *1 VU* is 1 Virtual User, *25 VU* is 25 Virtual Users, etc.).

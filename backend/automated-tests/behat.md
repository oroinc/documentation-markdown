...
* **Symfony2 Extension** provides integration with <a href="https://github.com/Behat/Symfony2Extension/blob/master/doc/index.rst" target="_blank">Symfony2</a>.
...
* <a href="https://sites.google.com/chromium.org/driver/" target="_blank">ChromeDriver</a> is an open-source tool for automated testing of web apps across many browsers. It provides capabilities for navigating to web pages, user input, JavaScript execution, and more.
...
- **Use form mapping instead of selectors in your scenarios** to keep them clear and understandable for people from both the technical and nontechnical world.
  ...
  **Do**:
  ...
  ```
- Use menu and links to get the right [pages](#behat-page-element) instead of the direct page URL
...
- **Use semantical yml fixtures**
  ...
  Use only the entities that are in the bundle you are testing. Any other entities should be included via import.  See [Alice fixtures](#behat-alice-fixtures) for more information.
...
#### HINT
See the [oro:install](../../bundles/platform/InstallerBundle/commands.md#bundle-docs-platform-installer-bundle-oro-install-command) command refference for more information.
...
Each feature can have alice fixtures, [added by tags](#behat-alice-fixtures). **FixturesChecker** will check every feature for ability to load fixtures, without actually loading the fixture.
...
To boost the database isolator, you can mount the database directory to RAM. In the illustration below, we use <a href="https://en.wikipedia.org/wiki/Tmpfs" target="_blank">tmpfs</a>:
...
![Grep flash messages in the console](img/backend/tests/grep_flash_messages.png)
```none
bin/behat -dl -s AcmeDemoBundle | grep "grid"
```

![Grep flash messages in the console](img/backend/tests/grep_grid.png)

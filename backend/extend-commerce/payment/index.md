...
As an example, let us implement a collect on delivery (cash on delivery, COD) payment option. This simple method does not use external services (like credit card payment interfaces) and needs only the minimum set of options to operate.
...
Check <a href="https://github.com/oroinc/orocommerce/blob/7.0/src/Oro/Bundle/PaymentBundle/Method/PaymentMethodInterface.php" target="_blank">PaymentMethodInterface</a> for more information on other predefined transactions.

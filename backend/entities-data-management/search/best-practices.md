...
<a href="http://www.elastichq.org/" target="_blank">Elastic HQ</a> provides the UI to manage the Elasticsearch cluster (indexes, mappings, queries etc) instead of the plain CLI. This plugin is recommended for use only in the development environment because of the possible security issues in the production environment.

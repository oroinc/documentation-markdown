...
This guide demonstrates how to set up [Docker and Symfony Server development stack](docker-and-symfony/index.md#setup-dev-env-docker-symfony) for Oro applications on Ubuntu 26.04 LTS.
...
1. Install php 8.5 with all required extensions:
   ...
   ```none
   sudo apt -y install php8.5-mongodb
   ```
   ...
   #### HINT
   MongoDB PHP Extension can be installed with [Symfony CLI](#ubuntu-install-symfony-server-and-enable-tls) from the application directory
   ```none
...
<a id="ubuntu-install-symfony-server-and-enable-tls"></a>
1. Install Symfony Server and enable TLS:
   ```none
   ...
   ```
2. Restart the terminal and web browser to get them ready.

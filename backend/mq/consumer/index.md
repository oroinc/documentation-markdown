...
To start the consumer as a background process, in most cases run the following command:

```none
php bin/console oro:message-queue:consume
```

The *oro:message-queue:transport:consume* command is a special case that lets you explicitly set a queue
...
#### NOTE
Consumption modes are available as of OroCommerce version 6.1.10.
...
#### NOTE
The configurable consumer `receive_timeout` option is available as of OroCommerce version 6.1.10.
...
The `--memory-limit` option is highly recommended for normal consumer usage, especially in production mode. If the
option is set, the consumer checks the used memory amount after processing each message and terminates if the limit is exceeded.
...
* If it exceeds the option value (i.e. 705 MB, 780 MB, or 1300 MB) the consumer is terminated (and Supervisord re-runs it)
* If not, the consumer continues to process the message.

We recommend always setting this option to a value 2-3 times lower than the PHP memory limit. This helps avoid PHP memory
limit errors during message processing.

We recommend setting the `--time-limit` option (e.g., `--time-limit='now+600 seconds`) to 5-10 minutes if you use the DBAL transport to avoid database connection issues.

## Interruption

A consumer normally interrupts message processing for several reasons:
...
Normal interruption occurs only after a message is processed. If an
event fires during message processing, the consumer finishes processing
the message and then interrupts.

A consumer also interrupts **if an exception is thrown during message
processing**.

## Heartbeat

The system can inform users about the state of consumers (whether at least one is alive) through the following process:
...
You can push a message intended for an external consumer into a message queue. Push it into a queue that the
Oro application consumer does not consume; otherwise `\Oro\Component\MessageQueue\Client\NoopMessageProcessor`
will likely process it.

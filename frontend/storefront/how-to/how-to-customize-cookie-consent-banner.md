<a id="frontend-how-to-customize-cookie-consent-banner"></a>

# How to Customize the Cookie Consent Banner

OroCommerce provides a built-in [cookie consent banner](../../../user/storefront/cookie-consent-banner/index.md#frontstore-guide-cookie-banner) through the [CookieConsentBannerBundle](../../../bundles/commerce/CookieConsentBundle/index.md#bundle-docs-commerce-cookie-consent-bundle). The banner displays a short message, an accept button, and a link to the cookie policy page.
...
#### NOTE
Before you start, [set up the development environment](../../../backend/setup/dev-environment/index.md#dev-guide-development-practice-setup-dev-env) with your own custom bundle, and [create a storefront theme](../theming.md#dev-doc-frontend-layouts-theming-definition).
...
The text and policy page link are set in the back-office, under **System > Configuration > Commerce > Customer > Customer Users > Cookies Banner**. No code is needed. See [Cookie Consent Banner](../../../user/storefront/cookie-consent-banner/index.md#frontstore-guide-cookie-banner) for details.
...
```scss
 $cookie-banner-view-background-color: get-color('primary', 'main');
 $cookie-banner-view-inner-offset: 20px 32px;
```
...
See [Stylesheets (SCSS)](../css/index.md#dev-doc-frontend-css) for more on the theme CSS structure.
...
```none
 <div class="banner-view-inner-container" data-skip-focus-decoration tabindex="0">
     <div class="cookie-banner-view__text cms-typography">
         <%= bannerText %>
         <% if (!_.isEmpty(landingPageHref) && !_.isEmpty(landingPageLabel)) { %>
             <a href="<%- landingPageHref %>" target="_blank">
                 <%- landingPageLabel %>
             </a>
         <% } %>
     </div>
     <div class="cookie-banner-view__buttons">
         <button data-action="accept" class="btn btn--info"
                 aria-label="<%- _.__('oro_cookie_banner.accept_button.aria_label') %>">
             <%- bannerButtonLabel %>
         </button>
     </div>
 </div>
 <button class="close-dialog" data-role="close" title="<%- _.__('Close') %>"
         aria-label="<%- _.__('oro_cookie_banner.close.aria_label') %>"></button>
```
...
The template receives `bannerText`, `bannerButtonLabel`, `landingPageHref`, and `landingPageLabel`. Keep the `[data-action="accept"]` and `[data-role="close"]` attributes - the view binds to them.
...
The banner is added to every storefront page as the `banner_content` block. Use [layout update](../layouts/index.md#dev-doc-frontend-layouts-layout) to move or remove it. For example, to remove it from the customer login page:
...
This example adds category checkboxes to the banner and saves the choice so other scripts can read it. It is a minimal version of the *Native OroCommerce or Custom Banner* approach from the [Cookie Consent concept guide](../../../user/concept-guides/administration/consents/cookie-consent.md#concept-guide-cookie-consent).

![The cookie consent banner extended with the Strictly necessary, Analytics, and Marketing checkboxes, and the Accept Necessary and Accept All buttons](user/img/storefront/how_to_customize_cookie_consent_banner/cookie_banner_categories.png)
...
```none
 <div class="banner-view-inner-container" data-skip-focus-decoration tabindex="0">
     <div class="cookie-banner-view__text cms-typography">
         <%= bannerText %>
         <% if (!_.isEmpty(landingPageHref) && !_.isEmpty(landingPageLabel)) { %>
             <a href="<%- landingPageHref %>"><%- landingPageLabel %></a>
         <% } %>
     </div>
     ...
     <div class="cookie-banner-view__buttons">
         <button data-action="reject" class="btn"
...
Other scripts can now read the accepted categories from local storage and react to the `cookie-consent:accepted` event. See [How to Add Third-Party Scripts to Storefront Pages](how-to-add-third-party-scripts.md#frontend-how-to-add-third-party-scripts-to-storefront) for details on how to load a script only after its category is accepted.

**Related Articles**

* [Cookie Consent in OroCommerce: Guidance for Merchants](../../../user/concept-guides/administration/consents/cookie-consent.md#concept-guide-cookie-consent)
* [Cookie Consent Banner (User Documentation)](../../../user/storefront/cookie-consent-banner/index.md#frontstore-guide-cookie-banner)
* [How to Add Third-Party Scripts to Storefront Pages](how-to-add-third-party-scripts.md#frontend-how-to-add-third-party-scripts-to-storefront)
* [Layouts](../layouts/index.md#dev-doc-frontend-layouts-layout)
* [JS Modules Configuration](../../../backend/configuration/yaml/jsmodules.md#reference-format-jsmodules)

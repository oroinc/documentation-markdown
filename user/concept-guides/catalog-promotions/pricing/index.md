...
Let’s now keep only two price lists in the chain, with the *Customer A PL* having a higher priority than the *Spring Sale PL*, and disable the Merge Allowed option for the *Spring Sale PL*:

|   **Priority** | **Price List**   | **Merge Allowed**   |
|----------------|------------------|---------------------|
|              1 | Customer A PL    | Yes                 |
|              2 | Spring Sale PL   | No                  |

The *Customer A PL* is processed first and provides the prices for the *1+*, *10+*, *20+*, and *50+* quantity tiers. When the system reaches the *Spring Sale PL*, the lumen headlamp already has prices coming from the higher priority price list. Since merging is not allowed for the *Spring Sale PL*, its prices are skipped entirely, and the empty *100+* tier remains unfilled:

| **Quantity Tier**   | **Customer A PL (merge: yes)**   | **Spring Sale PL (merge: no)**   | **Resulting Price**   |
|---------------------|----------------------------------|----------------------------------|-----------------------|
| 1+ items            | $85                              | $90                              | **$85**               |
...
However, if some product is listed in the *Spring Sale PL* but is missing from the *Customer A PL*, it gets all its price tiers from the *Spring Sale PL*, as no higher priority price list provides prices for it.

**Merge Allowed Is Disabled for the First Price List and Enabled for the Second One**

Now let’s invert the flags: the higher priority *Customer A PL* does not allow merging, while the *Spring Sale PL* does:

|   **Priority** | **Price List**   | **Merge Allowed**   |
|----------------|------------------|---------------------|
|              1 | Customer A PL    | No                  |
|              2 | Spring Sale PL   | Yes                 |

The *Customer A PL* is processed first. As the lumen headlamp has no prices yet, the price list provides its four price tiers, but because merging is not allowed, the product is closed for any further merging. Although the *Spring Sale PL* allows merging and defines the price for the *100+* tier, this price is not added, and the customer sees only the tiers from the *Customer A PL*:

| **Quantity Tier**   | **Customer A PL (merge: no)**   | **Spring Sale PL (merge: yes)**   | **Resulting Price**   |
|---------------------|---------------------------------|-----------------------------------|-----------------------|
| 1+ items            | $85                             | $90                               | **$85**               |
| 10+ items           | $82.45                          | $87.30                            | **$82.45**            |
| 20+ items           | $77.05                          | $83.70                            | **$77.05**            |
...
* A product missing from the *Customer A PL* still receives all its tiers from the *Spring Sale PL*.
...
#### NOTE
Custom configuration on the customer level has the highest priority, and it overrides configuration on the customer group level. Customer group configuration overrides configuration on the website level. The website level configuration overrides the global configuration settings.

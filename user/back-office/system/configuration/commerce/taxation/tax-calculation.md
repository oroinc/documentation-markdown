<a id="user-guide-taxes-tax-configuration"></a>
...
2. Click **Reset** at the top right to roll back any changes to the tax calculation settings.
...
#### HINT
For more information on price calculation precision and the different types of pricing rounding, refer to the [Pricing Settings](../catalog/global-pricing.md#sys-config-commerce-catalog-pricing-rounding) documentation.
...
* **Unit Price Formula** — This setting calculates taxes at the **unit level** before summing them up for the entire order. It multiplies the [rounded price](../catalog/global-pricing.md#sys-config-commerce-catalog-pricing-rounding) of a single unit by the tax rate and then by the quantity. This method ensures that each unit’s tax is computed individually. The parentheses `()` in the formula denote where rounding is applied.

| `Total Tax = [(Unit Price) x Tax Rate x Unit Quantity] + ... + [(Unit Price) x Tax Rate x Unit Quantity]`   |
|-------------------------------------------------------------------------------------------------------------|
* **Row Total Formula** — This setting calculates taxes based on the **row total** (the total price for all units of the product) rather than the unit price. It first multiplies the unit price by the quantity to get the total price for that product, rounds the result [based on the configuration](../catalog/global-pricing.md#sys-config-commerce-catalog-pricing-rounding) and then applies the tax rate. The parentheses `()` in the formula denote where rounding is applied.

| `Total Tax = [(Unit Price x Unit Quantity) x Tax Rate] + ... + [(Unit Price x Unit Quantity) x Tax Rate]`   |
|-------------------------------------------------------------------------------------------------------------|
![Start Calculation With configuration option](user/img/system/config_commerce/taxation/start-calculation-with.png)
...
![Start Calculation On configuration option](user/img/system/config_commerce/taxation/start-calculation-on.png)

Let’s illustrate the calculation and rounding logic using the following example:

| Product   | Price   |   Quantity | Tax Rate   | Rounding Precision   | Pricing Rounding Type   |
|-----------|---------|------------|------------|----------------------|-------------------------|
...
![Illustration showing tax calculation for products A,B, and C on the checkout storefront page following the Example 1 configuration](user/img/system/config_commerce/taxation/unit-price-item-config-sf.png)![Illustration showing tax calculation for products A,B, and C on the order view page in the back-office](user/img/system/config_commerce/taxation/unit-price-item-config-bo.png)
...
![Illustration showing tax calculation for products A,B, and C on the checkout storefront page following the Example 2 configuration](user/img/system/config_commerce/taxation/unit-price-total-config-sf.png)![Illustration showing tax calculation for products A,B, and C on the order view page in the back-office](user/img/system/config_commerce/taxation/unit-price-total-config-bo.png)
...
![Illustration showing tax calculation for products A,B, and C on the checkout storefront page following the Example 3 configuration](user/img/system/config_commerce/taxation/row-total-item-config-sf.png)![Illustration showing tax calculation for products A,B, and C on the order view page in the back-office](user/img/system/config_commerce/taxation/row-total-item-config-bo.png)
...
![Illustration showing tax calculation for products A,B, and C on the checkout storefront page following the Example 4 configuration](user/img/system/config_commerce/taxation/row-total-total-config-sf.png)![Illustration showing tax calculation for products A,B, and C on the order view page in the back-office](user/img/system/config_commerce/taxation/row-total-total-config-bo.png)
...
![Order summary showing Product A's tax-included price with no change to the final total](user/img/system/config_commerce/taxation/price-including-tax.png)
...
![Order summary showing Product A's tax-excluded price with taxes added on top of the price changing the final total](user/img/system/config_commerce/taxation/price-excluding-tax.png)
...
| Line Item   | Line Item Subtotal   | Discount Amount   | Tax Rate   | Rounding Precision   | Pricing Rounding Type   |
|-------------|----------------------|-------------------|------------|----------------------|-------------------------|

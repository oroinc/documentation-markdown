<a id="user-guide-widgets-sales-orders-volume"></a>
...
* **Date Range 2** — the second time period that displays the sales order volume over the selected period. To choose the date range, select the **Starting At** option in the drop-down and set the date to start the range. The chart uses the x-axis of Date Range 1 to illustrate its own period. Remember, that you specify only their starting date, while their end date is determined automatically by the duration of Date Range 1.

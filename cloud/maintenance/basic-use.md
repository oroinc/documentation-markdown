<a id="orocloud-maintenance-use"></a>
...
#### NOTE
When the currently running command is finished or stopped with a warning or a notice, the maintenance agent is automatically unlocked.
...
#### NOTE
If the application is already deployed, the command execution is restricted. Please contact the OroCloud or Oro Support team in case a full re-deploy from scratch is required.
...
#### NOTE
By default, upgrade commands are run with the –skip-search-reindexation option. However, you can use the –schedule-search-reindexation option if you require reindexation.
...
#### IMPORTANT
Upgrade commands remove patches applied using [orocloud-cli patch:apply](patches.md#orocloud-maintenance-patches) command. Use can confirm it interactively or using –skip-check-patches option.
...
#### NOTE
You will be prompted to enter a tag, branch or commit to clone the application source file from. Use a valid tag, branch or commit from the Oro application repository, for example, the <a href="https://github.com/oroinc/orocommerce-application/tree/1.6" target="_blank">1.6</a> branch, the|1.6.1| tag or the|d55c2f0| commit.

This command executes the following operations:

1. Enables the maintenance mode
2. Stops the services (consumers, cron, websocket)
3. Checks out the application code from the provided tag, branch or commit of the configured repository
4. Installs the external dependencies via the composer install
5. Performs oro:platform:update including assets
...
#### IMPORTANT
Do not drop tables, columns, entity fields; do not alter columns, entity fields types, please.
...
#### NOTE
You will be prompted to enter a tag, branch or commit to clone the application source file. Use a valid tag, branch or commit from the Oro application repository (for example, the <a href="https://github.com/oroinc/orocommerce-application/tree/1.6" target="_blank">1.6</a> branch, the <a href="https://github.com/oroinc/orocommerce-application/tree/1.6.1" target="_blank">1.6.1</a> tag or the|d55c2f0| commit).
...
3. Build assets
4. Performs oro:platform:update –skip-assets
5. Warmup layout caches
6. Restarts the related services (consumers, cron, websocket).
...
#### NOTE
You will be prompted to enter a tag, branch or commit to clone the application source file. Use a valid tag, branch or commit from the Oro application repository (for example, the <a href="https://github.com/oroinc/orocommerce-application/tree/1.6" target="_blank">1.6</a> branch, the <a href="https://github.com/oroinc/orocommerce-application/tree/1.6.1" target="_blank">1.6.1</a> tag or the <a href="https://github.com/oroinc/orocommerce-application/commit/d55c2f0110a524546ba9be33098bc409d8617b8f" target="_blank">d55c2f0</a> commit).
...
2. Installs the external dependencies via the composer install
3. Build assets
4. Warmup layout caches
5. Restarts the related services (consumers, cron, websocket).
...
#### IMPORTANT
The retention time for manually generated backups depends on the environment type. Backup retention follows the same policy as daily automated backups. The one-month retention described in the [OroCloud Architecture](../architecture/index.md#cloud-architecture) documentation applies only to production environments. For details on backup retention for different environment types, see [Environment Plans](../environments.md#cloud-environments).
...
#### NOTE
The {backup_date} argument is the one of the available backups listed in the backup:list command output, e.g., 2018-11-12-1425.
...
See [Sanitizing Configuration](advanced-use.md#orocloud-maintenance-advanced-use-sanitization-conf) for details on how to configure the sanitizing scope and strategy.
...
#### NOTE
Keep in mind that after restoration from the sanitized backup, all application users credentials become invalid. To reset a password for the admin user, use the orocloud-cli app:console “oro:user:update –user-password=new-admin-password admin” command.
...
Sometimes you may require to clear the application cache (for example, after applying a patch or changing a configuration). This can be done with the cache:rebuild command that rebuilds the application cache. If a [prebuilt application package](#orocloud-maintenance-use-upgrade) has been used to upgrade Oro applications in OroCloud the command is performed without downtime. This command does the following:
...
<a id="orocloud-maintenance-use-media-upload"></a>
...
#### NOTE
Credentials commands are not available in production (prod) environments.
...
#### NOTE
Commands are available with configured [OroGridFSConfigBundle](../../bundles/platform/GridFSConfigBundle/index.md#bundle-docs-platform-gridfs-config-bundle) only.
...
#### NOTE
The files in the source directory always overwrite the same files in the destination directory. Please always use *underscores* instead of *spaces* for the `source` directory name and for all file names too.

To upload media files and product images, use SFTP. It helps connect to the OroCommerce instance and transfer large amounts of data. To get SFTP access to your OroCloud instance, read the [related documentation](before-you-begin.md#sftp-access) and create a ticket in the support portal.
...
#### IMPORTANT
Once you have uploaded the images via FTP/SFTP and moved them to the right location for the image import, please run [images import via the UI](../../user/back-office/products/products/import-products.md#user-guide-import-product-images), as this assigns the images to the products and makes them available in the asset library.
...
#### NOTE
By default, the command runs in the `DRY-RUN` mode which means that no files will be transferred but displayed only for validation purposes. To perform media transfer, execute the command with the `--force` flag.
...
#### NOTE
The messages list is limited to 1000 records.
...
#### NOTE
The `vhost` and `queue` argument values can be retrieved with the rabbitmq:queue:list command.
...
#### NOTE
The `queue` argument value can be retrieved with the rabbitmq:queue:list command.

<a id="bundle-docs-commerce-pricing-bundle-search"></a>
...
Prices are added to the product search index by the event listeners of the `oro_website_search.event.index_entity.product` event. There is one listener per pricing storage, and only the listener of the active storage is enabled (see [Flat Price List Storage](flat-pricing-storage.md#bundle-docs-commerce-pricing-bundle-flat-pricing-storage) for details on the storage features):
...
| Action                                        | CPL storage entry point                                                                                              | Flat storage entry point                                                                                             |
|-----------------------------------------------|----------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
...
Finally, **switching the pricing storage** with the `oro:price-lists:switch-pricing-storage` command requires a manual full reindexation (`oro:website-search:reindex`), as described in [Enable Flat Pricing](../../../backend/setup/post-install/flat-pricing.md#dev-guide-setup-flat-pricing).
...
For the complete producer-to-consumer message paths, including the topic reference and the job structure of the build, see [Pricing Message Queue Flows](pricing-mq-flows.md#bundle-docs-commerce-pricing-bundle-mq-flows).
...
See the Search Indexation section of the [Flat Price List Storage](flat-pricing-storage.md#bundle-docs-commerce-pricing-bundle-flat-pricing-storage) article for details, and keep in mind that when the default price storage is replaced entirely (see [Price Storage](price-storage.md#bundle-docs-commerce-pricing-bundle-price-storage)), the default listeners keep indexing prices from the out-of-the-box entities — you need to disable them and index the prices from your storage instead, as well as trigger the `pricing` field group reindexation whenever the prices in your storage change.

**Related Topics**

* [Optimize Website Indexation and Price Recalculation](optimize-index-and-price-calculation.md#admin-website-index-and-price-calc) — operational guidance on running price recalculation and reindexation efficiently.
* [Combined Price Lists](combined-price-lists.md#bundle-docs-commerce-pricing-bundle-combined-price-lists) — the CPL build flow that produces the reindexation requests.
* [Flat Price List Storage](flat-pricing-storage.md#bundle-docs-commerce-pricing-bundle-flat-pricing-storage)

<a id="bundle-docs-platform-integration-bundle-webhooks"></a>

# Webhooks

#### NOTE
Webhook notification functionality is available as of OroCommerce version 6.1.9.
...
to a comma-separated list of relation names in <a href="https://jsonapi.org/format/#fetching-includes" target="_blank">JSON:API include format</a>.
...
2. Fill in the form:
   ...
   * **Format** (required): Select the webhook format from the drop-down ; the format controls how the outgoing request is built and the payload is structured (see [Webhook Formats]())
...
|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `default`    | Full JSON:API payload — sends `attributes`,<br/>`relationships`, and any `included` resources<br/>configured via `webhook_relations_includes`.                                                                                                                 |
| `thin`       | Thin payload — strips `attributes`,<br/>`relationships`, and `included` from the payload<br/>so only `type` and `id` are sent inside `data`.<br/>Useful when the receiver only needs to know that a<br/>change occurred and will fetch the full record itself. |

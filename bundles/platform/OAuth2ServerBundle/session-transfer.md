<!-- meta: description = Configure and use the Session Transfer grant to exchange an OAuth access token for a short-lived Oro browser session -->

<a id="bundle-docs-platform-oauth2-server-bundle-session-transfer"></a>
...
![Session Transfer sequence from OAuth access token exchange to an authenticated Oro browser session](img/bundles/OAuth2ServerBundle/session-transfer-flow.svg)
1. A client obtains an OAuth access token through one of the supported grants.
2. The client sends the access token to the `/oauth2-token` endpoint using the `session_transfer` grant.
3. The authorization server returns a one-time Session Transfer Token.
4. The client opens the Session Transfer endpoint on the target Oro site and passes the token in its query string.
5. Oro consumes the token, creates a browser session, and redirects the browser to the requested route.

## Configure an OAuth Application

Create or edit an OAuth application as described in [Manage OAuth Applications](../../../user/back-office/system/user-management/oauth-app.md#oauth-applications) or
[Manage Customer User OAuth Applications](../../../user/back-office/customers/customer-user-oauth-app/index.md#customer-user-oauth-app), and select **Enable Session Transfer**.
...
data. The command runs daily at midnight by default. See [OAuth2ServerBundle CLI commands](commands.md#bundle-docs-platform-oauth2-server-bundle-commands) for details.
